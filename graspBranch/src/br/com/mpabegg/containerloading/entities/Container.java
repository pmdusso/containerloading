...
public class Container {
	...
	private boolean BottomOverlap(int x, int y, Vector3d _pos) {
		...
		try {
			if (xMax > this.x || yMax > this.y) {
				throw new Exception(
						"bottomOverlap(int x, int y, Vector3d _pos) - Porra, saiu fora do container!");
			}
	...
	public Vector3d FallBox(Box bx, Vector3d _pos) {
		...
		try {
			if (pos.getZ() < 0) {
				throw new Exception(
						"FallBox(Box bx, Vector3d _pos) - Saiu fora do container!");
			}
			...
			while (true) {
				if (BottomOverlap(bx.getRelativeDimensions().getX(), bx
						.getRelativeDimensions().getY(), pos)) {
					lowerRef = pos.getZ();
	...
	public Vector3d getPosition(Box bx) {
		...
		Vector3d pos = new Vector3d(0, 0, this.z
				- bx.getRelativeDimensions().getZ());
	...
	public boolean fitsHere(Box bx, Vector3d pos) {
		try {
			...
			// Brute force check
			for (int i = pos.getX(); i < pos.getX()
					+ bx.getRelativeDimensions().getX(); i++) {
				for (int j = pos.getY(); j < pos.getY()
						+ bx.getRelativeDimensions().getY(); j++) {
					for (int k = pos.getZ(); k < pos.getZ()
							+ bx.getRelativeDimensions().getZ(); k++) {
	...
	public Box fitsIn(Box _box, Vector3d lastBoxInserted) {
		...
		if ((lastBoxInserted.getX() == this.x)
				|| ((this.x - lastBoxInserted.getX()) < _box
						.getRelativeDimensions().getX())) {

			lastBoxInserted = new Vector3d(0, lastBoxInserted.getY()
					+ _box.getRelativeDimensions().getY(),
					lastBoxInserted.getZ());
		}
		...
		// nao for
		// suficiente para a proxima caixa...
		if ((lastBoxInserted.getY() == this.y)
				|| ((this.y - lastBoxInserted.getY()) < _box
						.getRelativeDimensions().getY())) {
			lastBoxInserted = new Vector3d(0, // volta para o começo do
					// container
					0, lastBoxInserted.getZ()
							+ _box.getRelativeDimensions().getZ());
		}
		try {
					...
					for (int k = lastBoxInserted.getX(); k < this.x; k++) {
						...
						if (!this.spMatrix[k][j][i]) {
							if (fitsHere(_box, new Vector3d(k, j, i))) {
								_box.setRelativeCoordenates(new Vector3d(k, j,
										i));
								return _box;
							}
						}
	...
	public boolean insertBox(Box bx) {
		try {
			for (int i = bx.getRelativeCoordenates().getX(); i < bx
					.getRelativeCoordenates().getX()
					+ bx.getRelativeDimensions().getX(); i++) {
				for (int j = bx.getRelativeCoordenates().getY(); j < bx
						.getRelativeCoordenates().getY()
						+ bx.getRelativeDimensions().getY(); j++) {
					for (int k = bx.getRelativeCoordenates().getZ(); k < bx
							.getRelativeCoordenates().getZ()
							+ bx.getRelativeDimensions().getZ(); k++) {
						if (!this.spMatrix[i][j][k]) {
